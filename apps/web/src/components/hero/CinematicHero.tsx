...
export function CinematicHero() {
  return (
    <section className="relative min-h-[calc(100svh-5rem)] overflow-hidden border-b border-aph-gold/15">
      <HeroVideo
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100svh-5rem)] max-w-7xl items-center px-4 py-24 sm:px-6">
        <div className="max-w-3xl min-w-0">
          ...
          <motion.h1
            className="font-[family-name:var(--font-display)] text-4xl font-light leading-[0.98] text-aph-cream drop-shadow-[0_2px_18px_rgba(250,248,245,0.55)] sm:text-5xl md:text-6xl lg:text-7xl"
          ...
          <motion.div
            className="mt-12 grid max-w-2xl grid-cols-3 gap-3 border-t border-aph-gold/15 pt-6 sm:gap-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            {heroStats.map(([value, label]) => (
              <div key={value}>
                <p className="font-[family-name:var(--font-display)] text-2xl text-aph-gold sm:text-3xl">{value}</p>
                <p className="mt-1 text-[10px] uppercase tracking-wide text-aph-muted sm:text-xs sm:tracking-widest">{label}</p>
              </div>
            ))}
          </motion.div>
        </div>
